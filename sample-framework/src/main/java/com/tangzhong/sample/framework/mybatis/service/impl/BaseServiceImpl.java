 ...
 */
@Slf4j
public abstract class BaseServiceImpl<T extends BaseEntity, M extends BaseMapper<T>> extends ServiceImpl<M, T> implements IBaseService<T> {
    ...
    protected LambdaUpdateWrapper<T> updateWrapper(Long id){
        return new LambdaUpdateWrapper<>(getEntityClass()).eq(BaseEntity::getId, id);
    }
